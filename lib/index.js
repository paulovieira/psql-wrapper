...
internals.createPsqlOptionsStr = function (psqlOptions, wrapperOptions){
	...
	for(let key in psqlOptions){
		...
		// boolean options with false value should be ommited (otherwise psql will complain with the message:
		// "option '--xyz' doesn't allow an argument")
		if (psqlOptions[key] === false || psqlOptions[key] === 'false') {
			continue;
		}

		// make sure that wrapperOptions aren't used as psql options by mistake...
